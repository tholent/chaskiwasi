...
4. SMTP send via Fastmail submission. A **permanent refusal of the message** — a 5xx at `RCPT TO` (the recipient address no longer exists) or at `DATA` (the server will never accept this message) ⇒ ack `rejected_undeliverable`. Everything else is not an ack: the letter is left unacked to be retried. That covers a 4xx anywhere, an unreachable server (which ends the whole exchange in a 503, §4.1), **and a 5xx that refuses *us* rather than the letter** — `AUTH` or `MAIL FROM` — which is a guardian-fixable config fault, not a dead address. See A.11.
...
| V-23 | **Permanent send failure (A.11):** a 5xx at `RCPT TO`/`DATA` → ack `rejected_undeliverable`, terminal and idempotent on replay (no re-send); a 4xx, or a 5xx at `AUTH`/`MAIL FROM` → letter stays unacked and is retried, and still sends once the config is repaired |
...
**A.11 · A permanent SMTP rejection is a terminal ack; a transient one is not.** §4.7's original four statuses had no way to express "this address is permanently dead," so a 5xx left the letter unacked and the device retried it every sync forever, showing the child "on the road" for something that could never land. `rejected_undeliverable` closes that: a permanent refusal is terminal (device stops, surfaces "couldn't send — ask your guardians"), while a 4xx or an unreachable server stays transient and is retried, because losing a letter is the one outcome §4.7 refuses to buy and only a *permanent* refusal justifies giving up. The device needs no new logic — every ack was already terminal — so this is a wire addition, not a firmware behaviour change. (Test V-23.)

**Which 5xx counts is a question about phase, not code.** "A 5xx reply" was the first draft's shorthand and is too coarse to implement against: SMTP submission replies 5xx at four different phases, and only two of them are about the letter. A 5xx at `RCPT TO` or `DATA` condemns *this message* — no retry will ever deliver it, so it earns the terminal ack. A 5xx at `AUTH` or `MAIL FROM` is the server refusing *us*: a rotated app password, a sending quota, a disabled sender. Those are guardian-fixable, and they hit **every queued letter at once** — so treating them as permanent would terminally reject the child's entire outbox on the sync after a password rotation, and because rejects are recorded in the ack ring, repairing the password would not bring those letters back. That is precisely the silent unrecoverable loss §4.7 refuses to buy, arrived at from the opposite direction. The classification therefore belongs in `internal/mailbox`, which is the only layer that knows which phase replied; it reports permanence as `ErrUndeliverable` rather than handing a raw 5xx upward. (Tests V-23 and `TestSMTPSend_PhaseDecidesPermanence`.)
