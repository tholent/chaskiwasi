...
**Reconciliation — filing must not depend on uptime.** At startup and at the top of every sync, Wasi scans INBOX for any message whose sender does not resolve to a contact and MOVEs it to Held. Derivation (§5.2) never silently drops mail: if Wasi was down when a stranger's mail arrived, reconciliation quarantines it before the cursor can pass it by. (Test V-15.)
